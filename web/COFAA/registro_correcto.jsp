<%-- 
    Document   : registro_correcto
    Created on : May 14, 2017, 6:16:48 PM
    Author     : PsysacElrick
--%>

<html>
<!--head-->
    <head>
        <script type="text/javascript">
window.onload = function(){//window.addEventListener('load',function(){...}); (for Netscape) and window.attachEvent('onload',function(){...}); (for IE and Opera) also work
    alert("Se ha registrado correctamente la evaluación del profesor. Aún así, puede volver a evaluar las actividades que estén pendientes");
    
};
</script>
    </head>

<body> 

<jsp:include page="/COFAA/Apela.jsp"></jsp:include>

</body>
</html>
